...
<!-- UPDATE BOX -->
			<form
				...
				:class="{
					'updatebox':true,
					'c-updatebox':true,
					'c-updatebox--mobile':(get.mobile),
					'c-updatebox--updating-tags': item.id == state.editingItemTags
					}"
			...
			>
				<div :class="{
					...
					'c-updatebox__body--mobile':(get.mobile)
					}"
				>
					<textarea
				...
				<div :class="{ 'c-update-tags':true,
					'c-updatebox__lower-body--mobile':(get.mobile)}"
				>
					<div
						:class="{ 'c-update-planned-time':true,
							'c-update-planned-time--mobile':(get.mobile)}"
						v-show="state.selection.view != 'journal'"
					>
						{{ get.text.card.duration }}
						<button
							:class="{ 'o-btn':true,
							...
							@blur="blurOnEdit(item)"
							value="10"
						>10{{ (get.mobileSmall) ? get.text.global.m : get.text.global.min }}</button>
						<button
							v-if="!get.mobileSmall"
							:class="{ 'o-btn':true,
								'c-update-planned-time__button':true,
							...
							@blur="blurOnEdit(item)"
							value="15"
						>15{{ (get.mobileSmall) ? get.text.global.m : get.text.global.min }}</button>
						<button
							:class="{ 'o-btn':true,
								'c-update-planned-time__button':true,
							...
							@blur="blurOnEdit(item)"
							value="30"
						>30{{ (get.mobileSmall) ? get.text.global.m : get.text.global.min }}</button>
						<button
							:class="{ 'o-btn':true,
								'c-update-planned-time__button':true,
							...
							@blur="blurOnEdit(item)"
							value="60"
						>1{{ (get.mobileSmall) ? get.text.global.h : get.text.global.hour }}</button>
						<div><input
							...
							@keydown="keydownOnEdit(item, $event, 'planned-time')"
							type="number"
						/>{{ get.text.global.min }}</div>
					</div>
					<div :class="{'c-item-tags':true,
						'c-updatebox__item-tags':true,
						'c-updatebox__item-tags--mobile':(get.mobile)}"
						v-if="get.mobile && (item.id == state.editingItem || item.id == state.editingItemTags)"
					>
						<div
						...
						<button v-if="get.mobile" class="c-mobile-edit-save-btns o-btn" @click="doneEdit(item)">
							{{ get.text.global.save }}
						</button>
					</div>
				</div>
			</form>
...
<!-- ADD NEW BOX -->
	<form 
		:class="{
			'addnewbox':true, 
			'c-addnewbox':true, 
			'c-addnewbox--mobile':(get.mobile),
			'child':addingNewAsChild,
			'first-child':addingNewAsFirstChild,}"
		:id="'new-under-'+ item.id "
		v-if=" showAddNewBox
			|| (listIsEmpty && !get.mobile && state.selection.view != 'journal')"
		@submit.prevent
		@click="clickOnAddNewCurtain($event)"
	>
		<div :class="{
			'c-addnewbox__body':true,
			'c-addnewbox__body--mobile':(get.mobile),
			'flex-wrap':true,
			}"
		>
			<div class="c-language-picker w-100" v-if="listIsEmpty && get.mobile">
				<a href="#"
		...
		<div :class="{
			'c-update-tags':true,
			'c-addnewbox__lower-body--mobile':(get.mobile),
			}"
		>
			...
			<div :class="{ 'c-item-tags':true,
				...
				'c-prepared-tags--mobile':(get.mobile) }"
			>
			...
			<div class="c-mobile-buttonrow" v-if="get.mobile">
				<button class="o-btn" @click="cancelAddNew" v-if="!listIsEmpty">{{ get.text.global.cancel }}</button>
				<button class="o-btn" @click="addNew">{{ get.text.card.addAndContinue }}</button>
				<button class="o-btn" @click="addNew('stop')">{{ get.text.card.addAndClose }}</button>
			</div>
		</div>
	</form>
<!-- / ADD NEW BOX -->
...
<script>
import { linkify, momentCalendar, sec_to_hourminsec, sec_to_hourmin } from '../helpers/valueMorphers2.js';
...
| ◉  |	FROM THE STORE 								|  ◉ |
 \* / ============================================== \ */
		...
		addingNewAsFirstChild(){ return this.state.addingNewAsFirstChild; },
		addingNewAsChild(){ return this.state.addingNewAsChild; },
	...
	methods: {
		...
		newItemIndent()
		{
			...
			{ // If item has no children yet / no visible children
				this.state.addingNewAsChild = true;
				return;
			}
		...
		newItemUnindent()
		{
			if (this.state.addingNewAsChild)
			{
				this.state.addingNewAsChild = false;
				this.state.addingNewAsFirstChild = false;
				return;
			}
		...
		keydownOnEdit(item, e, field)
		{
			...
			if (e.keyCode === 38)
			{
				...
				{
					e.preventDefault();
		        	if (this.state.editingItemTags){ this.state.editingItemTags = null; return; }
			...
			if (e.keyCode === 40)
			{
				...
				if (field == 'add-tag')
				{
		        	if (this.state.editingItemTags){ this.state.editingItemTags = null; }
		        	return;
				}
			}
			// ESC
		...
		deleteTag(id, tagName, event)
		{
			...
			document.querySelector(plsFocus).focus();			
			this.dispatch('patchTag',{id:id, tags:tagName, requestType:'untag'});
		},
