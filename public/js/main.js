...
exports.sec_to_hourmin = sec_to_hourmin;
...
},{"./components/globalFunctions.js":30,"./vue-components/VueListMaster.js":37,"./vue-components/dataTree.js":38,"./vue-components/vueFilters.js":39,"flatpickr":2,"jquery":3,"linkifyjs":5,"linkifyjs/html":4,"linkifyjs/plugins/hashtag":24,"vue":29,"vue-autosize":26,"vue-resource":28}],32:[function(require,module,exports){
'use strict';
...
var _PopoutConfirmation = require('./PopoutConfirmation.vue');

var _PopoutConfirmation2 = _interopRequireDefault(_PopoutConfirmation);
...
exports.default = {
	...
	props: ['popouts'],
	components: {
		PopoutConfirmation: _PopoutConfirmation2.default
	},
	data: function data() {
		return {
			timerRunning: true
	...
	methods: {
		...
		timerNav: function timerNav(button, item) {
			if (button == 'play') {
				this.playTimer(item);
			}
			if (button == 'pause') {
...
if (module.exports.__esModule) module.exports = module.exports.default
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n<div id=\"popouts-mask\" v-if=\"popouts.timer.length || popouts.delete.length\" @click=\"clearAll($event)\">\n<!-- \t<Popout-Confirmation\n\t\tv-if=\"popouts.delete.length\"\n\t\tv-for=\"item in popouts.delete\"\n\t\t:item=\"item\"\n\t></Popout-Confirmation> -->\n\n\t<div v-if=\"popouts.delete.length\" v-for=\"item in popouts.delete\" class=\"popout\">\n\t\t<div class=\"delete\">\n\t\t\t<div class=\"bodybox\">Really delete「{{ item.body }}」<strong v-if=\"item.children.length\" style=\"padding-left:0 0.3em;\"> and all children</strong>?\n\t\t\t</div>\n\t\t\t<div class=\"nav\">\n\t\t\t\t<button class=\"btn-cancel\" @click=\"popoutCall('confirm-cancel', item)\">Cancel</button>\n\t\t\t\t<button class=\"btn-ok\" @click=\"popoutCall('confirm-ok', item)\">OK</button>\n\t\t\t</div>\n\t\t</div>\n\t</div>\n\t<div v-if=\"popouts.timer.length\" v-for=\"item in popouts.timer\" class=\"popout\">\n\n\t\t<div :class=\"{\n\t\t\t\tdone: item.done,\n\t\t\t\ttimer: true,\n\t\t\t}\">\n\t\t\t<div class=\"body\">\n\t\t\t\t<div class=\"bodybox\">\n\t\t\t\t\t<div class=\"toggle-div\">\n\t\t\t\t\t\t<input class=\"toggle\" type=\"checkbox\" v-if=\"item.children_order.length==0 || item.done == true\" v-model=\"item.done\" @change=\"updateDone(item)\">\n\t\t\t\t\t</div>{{ item.body }}\n\t\t\t\t</div>\n\t\t\t\t<div v-if=\"!item.planned_time\" class=\"timer-time\">\n\t\t\t\t<!-- {{ item.used_time | hhmmss }}</div> -->\n\t\t\t\t{{ item.used_time }}</div>\n\t\t\t\t<div v-if=\"item.planned_time\" class=\"timer-time countdown\">\n\t\t\t\t<!-- {{ popout.id | countdown }}</div> -->\n\t\t\t\t{{ item.id }}</div>\n\t\t\t</div>\n\t\t\t<div class=\"nav\">\n\t\t\t\t<button class=\"play btn btn-dipclick\" v-show=\"!timerRunning\" @click=\"timerNav('play', item)\"><i class=\"zmdi zmdi-play\"></i>\n\t\t\t\t</button>\n\t\t\t\t<button class=\"pause btn btn-dipclick\" v-show=\"timerRunning\" @click=\"timerNav('pause', item)\"><i class=\"zmdi zmdi-pause\"></i>\n\t\t\t\t</button>\n\t\t\t\t<button class=\"forward btn btn-dipclick\" @click=\"timerNav('forward', item)\">+1 min\n\t\t\t\t</button>\n\t\t\t\t<button class=\"reset btn btn-dipclick\" @click=\"timerNav('reset', item)\">Reset</button>\n\t\t\t\t<button class=\"btn-ok\" @click=\"timerNav('close', item)\">OK</button>\n\t\t\t</div>\n\t\t</div>\n\t</div>\n</div>\n"
if (module.hot) {(function () {  module.hot.accept()
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
...
},{"./PopoutConfirmation.vue":33,"vue":29,"vue-hot-reload-api":27}],35:[function(require,module,exports){
'use strict';
...
exports.default = {
    ...
    methods: {
        ...
        removePopup: function removePopup(popup) {
            ...
            var index = root.popups.indexOf(popup);
...
var _dataTree = require('./dataTree.js');
...
exports.default = {
	...
	computed: {
		...
		totalSecLeft: function totalSecLeft() {
			if (!this.allData || !this.$children.length) {
				return 0;
			}
			console.log('run totalSecLeft');
			var card = this.$children.find(function (child) {
				return child.totalSecLeft || child.totalSecLeft == 0;
			});
		...
		totalUsedSec: function totalUsedSec() {
			...
			var card = this.$children.find(function (child) {
				return child.totalUsedSec || child.totalUsedSec == 0;
			});
			return card.totalUsedSec;
		},
		totalUsedHourMin: function totalUsedHourMin() {
			return (0, _globalFunctions.sec_to_hourmin)(this.totalUsedSec);
		},
		totalHourMinLeft: function totalHourMinLeft() {
			return (0, _globalFunctions.sec_to_hourmin)(this.totalSecLeft);
		}
	},
	methods: {
   ...
   */
		...
		filterItems: function filterItems(keyword, value, event) {
			...
			if (keyword == 'tag') {
				...
				if (keyword == 'journal') {
					if (selection.view.includes('journal')) {
						return;
					}
...
},{"../components/globalFunctions.js":30,"./Card.vue":32,"./Popouts.vue":34,"./Popups.vue":35,"./Selection.js":36,"./dataTree.js":38}],38:[function(require,module,exports){
'use strict';
...
var Tree = function () {
	...
	_createClass(Tree, [{
		...
		value: function filterItems(keyword, value, operator) {
			...
			if (keyword == 'duedate') {
				var dueDateFilterResults = this.arrayFilterDate(arrayToFilter, value, operator);
				Array.prototype.push.apply(filteredArray, dueDateFilterResults);
			}
			if (keyword == 'tag') {
				var tagFilterResults = this.arrayFilterTag(arrayToFilter, value, operator);
		...
		key: 'arrayFilterDone',
		value: function arrayFilterDone(array, operator) {
			var filteredArray = [];
			array.forEach(function (item) {
				if (item.done) {
					item.show_children = 1;
		...
		key: 'arrayFilterDate',
		value: function arrayFilterDate(array, date, operator) {
			var filteredArray = [];
			if (date == 'today') {
				array.forEach(function (item) {
...
},{"../components/globalFunctions.js":30}],39:[function(require,module,exports){
'use strict';
